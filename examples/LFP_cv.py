...
for train_index, val_index in kf.split(prepared_data):
    train_size = 0.9
    ...
    print(len(train_cells), len(val_cells), len(test_cells))
...
#     plt.savefig(f'media/LFP/img/m2_parity_in_{input_window_size}_out_{output_window_size}_window.png', dpi=400, transparent=False, bbox_inches="tight")
    ...
    mae_slopes_list.append(slope_mae)
    rmse_dc_list.append(dc_rmse)
#     break


dict_results = {'input_window_size':input_window_size,
                    'prediction_window_size':output_window_size,
                    'mean_test_slope_MAE':np.mean(mae_slopes_list),
                    'std_test_slope_MAE':np.std(mae_slopes_list),
                    'mean_test_dc_RMSE': np.mean(rmse_dc_list),
                    'std_test_dc_RMSE': np.std(rmse_dc_list)
                   }
import json
with open(f'CACHED_DATA/results/LFP/input_window_{input_window_size}_output_window_{output_window_size}_CV.json', 'w') as f:
